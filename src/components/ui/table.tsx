...
>(({ className, ...props }, ref) => (
  <tfoot
    ref={ref}
    className={cn(
      "border-t border-border bg-muted/60 font-semibold tabular-nums [&>tr]:last:border-b-0",
      className,
    )}
    {...props}
  />
...
const TableRow = React.forwardRef<HTMLTableRowElement, React.HTMLAttributes<HTMLTableRowElement>>(
  ({ className, ...props }, ref) => (
    <tr
      ref={ref}
      className={cn(
        "border-b border-border-soft transition-colors hover:bg-primary/[0.04] data-[state=selected]:bg-primary/[0.07]",
        className,
      )}
      {...props}
    />
