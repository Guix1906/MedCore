...
export default function PaymentHistory({
  ...
  const receive = async (event: React.FormEvent) => {
    ...
    try {
      ...
      toast.success("Baixa registrada. Nenhuma transferência bancária foi executada.");
    } catch (error) {
    ...
    >
      <section className="mx-auto max-w-3xl rounded-2xl bg-white p-6 space-y-4">
        <div className="flex justify-between print:hidden">
          <h2 className="font-bold">Baixas e histórico</h2>
          <button disabled={busy || submitted} onClick={onClose}>
            Fechar
          </button>
        </div>
        ...
        {title.can_settle &&
          ...
          title.status !== "cancelado" && (
            <form onSubmit={receive} className="rounded-xl border p-4 space-y-3 print:hidden">
                  ...
                  >
                    ...
                    {accounts.map((a) => (
                      <option key={a.id} value={a.id}>
                        {a.name}
                        {!a.balance_kind ? " (classificar no Fluxo de caixa)" : ""}
                      </option>
                    ))}
                  </select>
                </label>
              ...
              {accounts.length === 0 && (
                <p role="alert">Cadastre uma conta ativa desta clínica na aba Contas.</p>
              )}
            </form>
          )}
        <div className="space-y-3 print:hidden">
          {payments.length === 0 && <p>Nenhuma baixa registrada.</p>}
          {payments.map((p) => (
        ...
        {reversing && (
          <form onSubmit={reverse} className="space-y-2 border p-3 print:hidden">
            ...
            <button type="button" disabled={busy} className="ml-3" onClick={() => setReversing("")}>
              Cancelar
            </button>
          </form>
        )}
        {receipt && (
          <section id="financial-receipt" className="rounded-lg border p-5 space-y-2">
            ...
            <p>Pagador / favorecido: {receipt.payer_name || title.payer_name || "Não informado"}</p>
            <p>
